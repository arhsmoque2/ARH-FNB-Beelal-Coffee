...
if (fs.existsSync(mainWrangler)) {
  try {
    ...
    const hasR2 = r2List.some(
      (b) => b.binding === "MEDIA_BUCKET" && b.bucket_name === "arh-fnb-beelal-media"
    );
    if (hasR2) {
...
if (fs.existsSync(billingWrangler) && fs.existsSync(billingSchema)) {
  try {
    ...
    const hasD1 = d1List.some(
      (db) => db.binding === "DB" && db.database_name === "fnb-billing-ledger-db"
    );
    if (hasD1) {

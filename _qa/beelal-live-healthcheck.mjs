...
const targetUrl = (process.argv[2] || process.env.TARGET_URL || 'https://store-beelal-fnb-pwa.arh-homelab.workers.dev').replace(/\/+$/, '');
...
async function checkRoute(path, expectedStatuses = [200], validateFn = null) {
  ...
  try {
    ...
    if (statusOk && customOk) {
      console.log('  ✅ [PASS] ' + path.padEnd(20) + ' HTTP ' + res.status + ' (' + text.length + ' bytes)');
    } else {
      failed++;
...
async function run() {
  ...
  await checkRoute('/index-v2.html', [200], (text) => {
    if (!text.includes('cartList') || !text.includes('APP_CONFIG')) return 'Missing storefront markup/config';
  });

  await checkRoute('/admin.html', [200], (text) => {
    if (!text.includes('loadOrders') || !text.includes('Admin')) return 'Missing admin panel components';
  });
  ...
  const fbUrl = 'https://ash-2026-photobook-default-rtdb.asia-southeast1.firebasedatabase.app/beelal_coffee/config.json';
  try {
    const fbRes = await fetch(fbUrl, { headers: { 'User-Agent': 'Beelal-HealthCheck/1.0' } });
    if (fbRes.ok) {
      const fbData = await fbRes.json();
      console.log('  ✅ [PASS] Firebase RTDB read HTTP ' + fbRes.status + ' (Store: ' + (fbData?.store_info?.name || 'beelal_coffee') + ')');
    } else {
      failed++;

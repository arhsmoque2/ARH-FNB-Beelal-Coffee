  ...
  for (const vp of VIEWPORTS) {
    ...
    try {
      ...
      if (!response || !response.ok()) {
        console.warn(
          `  ⚠️ Page returned HTTP ${response ? response.status() : "null"}, retrying with domcontentloaded...`
        );
      ...
      const overlaps = await page.evaluate(() => {
        const selector =
          "button, input, select, .item-card, .menu-card, .quick-card, .brand, .topbar-inner > *, .category-strip button, .category-chip, .floating-cart";
            ...
            if (hasIntersection) {
              ...
              if (overlapArea > 32) {
                ...
                collisionList.push({
                  ...
                  rectA: {
                    left: Math.round(rA.left),
                    top: Math.round(rA.top),
      ...
      if (overlaps.length === 0) {
        ...
        for (const o of overlaps.slice(0, 5)) {
          console.error(
            `     - Collision: ${o.elementA} overlaps with ${o.elementB} (${o.overlapPixels}px²)`
          );
        }
      ...
      const hitTestResults = await page.evaluate(() => {
        const interactiveButtons = Array.from(
          document.querySelectorAll(
            'button:not([disabled]), a[href], .floating-cart, input[type="search"]'
          )
        ).filter((el) => {
          const style = window.getComputedStyle(el);
          if (style.display === "none" || style.visibility === "hidden" || style.opacity === "0")
        ...
        for (const btn of interactiveButtons) {
          ...
          const isAccessible =
            btn === topElement || btn.contains(topElement) || topElement.contains(btn);
          if (!isAccessible) {
            const getLabel = (el) =>
      ...
      if (hitTestResults.length === 0) {
        console.log(
          "  ✅ [Hit-Testing] 0 click obstructions. All interactive targets hit-tested cleanly."
        );
      } else {
        console.error(
          `  ❌ [Hit-Testing] Found ${hitTestResults.length} obstructed click target(s):`
        );
        for (const h of hitTestResults) {
          console.error(
            `     - Target ${h.target} is obstructed by ${h.blockedBy} at (${h.coords.x}, ${h.coords.y})`
          );
        }
      ...
      const whitespaceResults = await page.evaluate((maxGap) => {
        ...
        const blocks = Array.from(
          document.querySelectorAll(
            ".topbar, .hero, .quick-row, .category-strip, .items-grid, .side-panel, footer, .footer"
          )
        )
          .map((el) => {
            const style = window.getComputedStyle(el);
      ...
      if (whitespaceResults.abnormalGaps.length === 0) {
        console.log(
          `  ✅ [Whitespace Check] 0 abnormal dead spaces (Threshold: <= ${vp.maxBlankGap}px).`
        );
      } else {
        console.warn(
          `  ⚠️ [Whitespace Check] Detected ${whitespaceResults.abnormalGaps.length} large blank space gap(s):`
        );
        for (const g of whitespaceResults.abnormalGaps) {
          console.warn(
            `     - Dead Space: ${g.gapPixels}px gap between ${g.between} (max: ${g.maxAllowed}px)`
          );
        }
